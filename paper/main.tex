...
% Structured authorship (RRP-0021/0025/0026): mirrors rrxiv-meta.json.
...
A paper arguing that retraction should be a first-class annotation owes the reader a working example. This revision therefore includes a deliberate one: claim c7 (Section~\ref{sec:demo}) is written to be retracted, and the repository ships the corresponding \texttt{claim\_retraction} annotation record (RRP-0020) that withdraws it in favour of claim c5. The retraction machinery this paper proposes is thereby exercised, end to end, on the paper itself.

Section~\ref{sec:background} surveys what existing infrastructure does and why. Section~\ref{sec:approach} describes the annotation schema and the audit procedure. Section~\ref{sec:results} presents the registered claims, including the live retraction demonstration. Section~\ref{sec:discussion} discusses the policy implications --- in particular, what a retraction policy looks like when retractions are first-class graph edges rather than database flags.
...
\begin{claim}[type=theoretical, evidence=argument, labels={retraction, metascience}, title=Claim 1]
\label{claim:c1}
...
Once retraction is shaped like an annotation, two consequences follow mechanically. Notices can target claims rather than papers (Section~\ref{sec:approach}), and notices can carry typed metadata in fields rather than free text, making them queryable. The same logic applies to author corrigenda, which are presently encoded as separate documents but are structurally identical to a retraction with reason \texttt{data\_error} and scope restricted to a single claim.

\subsection*{Claim 2: most retracted papers contain survivor claims}
\begin{claim}[type=empirical, evidence=observation, labels={retraction, metascience, load-bearing}, datasets={PubMed retraction notices 2020--2024}, title=Claim 2]
\label{claim:c2}
...
\begin{claim}[type=computational, evidence=simulation, labels={retraction, metascience}, regimes={synthetic citation graph of 12000 papers and 96000 claims}, title=Claim 3]
\label{claim:c3}
...
\begin{claim}[type=empirical, evidence=observation, labels={retraction, metascience}, datasets={PubMed retraction notices 2010--2024}, title=Claim 4]
\label{claim:c4}
...
\begin{claim}[type=theoretical, evidence=argument, labels={retraction, metascience}, title=Claim 5]
\label{claim:c5}
...
% Cross-paper edge: the append-only-citation-graph vs revisable-annotation-layer
% distinction this claim relies on is developed in rrxiv:2605.00006 c4
% (see Section 2 and the [rrxivannotation] reference note).
\dependson{rrxiv:2605.00007:claim:c5}{rrxiv:2605.00006:claim:c4}
...
\begin{claim}[type=theoretical, evidence=argument, labels={retraction, metascience}, title=Claim 6]
\label{claim:c6}
...
% Cross-paper edge: the propose-then-confirm discipline for agent-issued
% retraction machinery established in rrxiv:2605.00005 c6 is generalised here
% to agent-proposed supersession edges (see the paragraph below and the
% rrxivremark that follows).
\extendsclaim{rrxiv:2605.00007:claim:c6}{rrxiv:2605.00005:claim:c6}
...
\subsection*{Claim 7: a live retraction demonstration}
\label{sec:demo}

\begin{claim}[type=theoretical, evidence=argument, confidence=0.05, rationale={Included as a live retraction demonstration; withdrawn by the prepared claim-retraction annotation record in the paper repository}, labels={retraction, metascience, demo-retraction}, title=Claim 7 (retracted by design)]
\label{claim:c7}
...
\end{claim}
\contradicts{rrxiv:2605.00007:claim:c7}{rrxiv:2605.00007:claim:c5}

\emph{This claim is intentionally retracted-as-demonstration; see the annotation record.} A paper whose thesis is that retraction should be a first-class, claim-scoped annotation ought to exercise that machinery at least once, on itself. Claim c7 states the status-quo position this paper argues against --- Section~\ref{sec:results} (c5) shows the projection from claim-level annotations to a paper-level bit is a one-way information loss --- and exists precisely so that it can be withdrawn through the sanctioned author fast path of RRP-0020. The paper repository ships the prepared annotation record at \texttt{annotations/claim-retraction.demo.json}: a \texttt{claim\_retraction} annotation targeting c7 with reason \texttt{withdrawn\_by\_author}, \texttt{superseded\_by\_claim} pointing at c5, and \texttt{recommended\_action: use\_superseded\_by}. Once posted to the canonical instance by the paper's authors, the server derives c7's \texttt{replication\_status} as \texttt{retracted} while c1--c6 remain live --- a one-claim, in-vivo instance of the survivor-claim situation that claim c2 measures in the wild. Readers encountering c7 through a protocol-aware client should see the retraction banner, the reason, and the pointer to c5; readers of this PDF see this paragraph. That asymmetry is the paper's argument in miniature.
...
\item \textbf{[rrxivwhitepaper]} Albis-Burdige, B.\ and Claude, 2026. \emph{rrxiv: a reproducibility-first scholarly protocol.} \texttt{rrxiv:2605.00001}. The genesis paper; RRP-0020 specifies the wire format for author claim retraction, which this paper extends to third-party retraction.
...
\item \textbf{[rrxivannotation]} Albis-Burdige, B.\ and Claude, 2026. \emph{Citation graphs are not knowledge graphs.} \texttt{rrxiv:2605.00006}. Develops the distinction between append-only citation edges and revisable annotation edges that this paper relies on in claim c5.
\item \textbf{[rrxivagents]} Albis-Burdige, B.\ and Claude, 2026. \emph{On the editorial role of agents in preprint commentary.} \texttt{rrxiv:2605.00005}. Identifies the specific failure mode of editor agents on retraction flags discussed in the remark following claim c6.
\end{itemize}
\end{document}
